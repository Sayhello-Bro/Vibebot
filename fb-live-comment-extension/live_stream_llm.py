...
STREAM_SETTINGS_COLLECTION_NAME = os.environ.get("MONGODB_STREAM_SETTINGS_COLLECTION", "stream_reply_settings")
...
stream_settings_collection = db[STREAM_SETTINGS_COLLECTION_NAME]
...
def serialize_reply(source: str, document: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not isinstance(embedding, list) or not embedding:
        try:
            ...
            collection = default_collection if source == "default" else user_collection
            collection.update_one(
                {"_id": document["_id"]},
                {"$set": {"embedding": embedding, "embedding_pending": False}},
            )
        except Exception:
            return None
    return {
...
def get_stream_enabled_keys(stream_id: str) -> set[str]:
    setting = stream_settings_collection.find_one({"stream_id": stream_id})
    if setting is None:
        return {reply["key"] for reply in CACHED_REPLIES if reply.get("enabled", True)}
    return {str(key) for key in setting.get("enabled_keys", [])}


def get_stream_reply_mode(stream_id: str) -> str:
    setting = stream_settings_collection.find_one({"stream_id": stream_id}) or {}
    mode = str(setting.get("reply_mode") or "fixed").strip().lower()
    return mode if mode in {"fixed", "auto"} else "fixed"


def build_candidates(raw_embedding: list[float], stream_id: str) -> list[dict[str, Any]]:
    candidates: list[dict[str, Any]] = []
    enabled_keys = get_stream_enabled_keys(stream_id)
    for reply in CACHED_REPLIES:
        if reply["key"] not in enabled_keys:
            continue
...
def choose_for_accounts(raw_text: str, account_ids: list[str], stream_id: str,
    ...
    candidates = build_candidates(raw_embedding, stream_id)
    context = context or {}
    ...
    policy_allows_reply = policy["action"] == "reply" or (
        policy["action"] == "uncertain" and reply_mode == "auto"
    )
    if reply_mode == "auto" and policy_allows_reply and cooldown_remaining <= 0:
        try:
            ...
            if generation.get("valid") and generation.get("reply"):
                ...
                generated_candidate = {
                    "key": f"generated:{stream_id}:{time.time_ns()}",
                    "source": "qwen_generated", "id": None,
                    "text": generation["reply"], "similarity": None,
                    "weight": 1.0, "score": None, "multi_output": True,
                }
    ...
    if reply_mode == "auto":
        usable_candidates = [generated_candidate] if generated_candidate else []
    else:
    ...
    for account_id in account_ids:
        ...
        for candidate in usable_candidates:
            ...
            if not already_used:
                selected = candidate
                break
            if candidate.get("source") == "qwen_generated":
                selected = candidate
                break
...
def process_raw_text(raw_text: str, stream_id: str, account_ids: list[str], extra: dict[str, Any] | None = None) -> dict[str, Any]:
    chosen = choose_for_accounts(raw_text, account_ids, stream_id, extra)
    result = {
...
def stream_replies():
    try:
        ...
        setting = stream_settings_collection.find_one({"stream_id": stream_id})
        if setting is None:
            enabled_keys = {reply["key"] for reply in CACHED_REPLIES if reply.get("enabled", True)}
            reply_mode = "fixed"
        else:
        ...
        if request.method == "PATCH":
            ...
            if requested_mode:
                if requested_mode not in {"fixed", "auto"}:
                    return jsonify({"status": "error", "error": "reply_mode must be fixed or auto"}), 400
            ...
            stream_settings_collection.update_one(
                {"stream_id": stream_id},
                {"$set": {"enabled_keys": sorted(enabled_keys), "reply_mode": reply_mode,
