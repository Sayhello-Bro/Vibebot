    ...
    foreach ($line in Get-Content -LiteralPath $JsonlPath -Encoding UTF8) {
        ...
        try {
            ...
            if ($AccountMode -eq "qwen_only") {
                ...
                if (
                    [bool]$result.qwen_called -and
                    [int]$result.crowd_slogan_count -ne $DesiredCandidateCount
                ) {
            ...
            Write-Host (
                "向量執行={0}，向量候選={1}，自訂語句={2}，衝人氣口號={3}，Qwen呼叫={4}，模型累計字數={5}/{6}，等待合併={7}" -f `
            ...
            foreach ($accountResult in @($result.account_results)) {
                ...
                if (
                    ...
                    [string]$accountResult.reply_source -notin @(
                        "qwen", "qwen_ignore", "qwen_crowd", "qwen_frequency_fallback", "model_input_pending"
                    )
                ) {
            ...
            if ($null -ne $result.generation) {
                Write-Host (
                    "Qwen批次結果={0} 筆 | Qwen時間={1:N2} ms | 輸入token={2} | 輸出token={3} | 自訂語句={4} | 衝人氣口號={5}" -f `
                    ...
                    [int]$result.generation.crowd_slogan_count_sent
                ) -ForegroundColor DarkGray
            }
            Write-Host (
