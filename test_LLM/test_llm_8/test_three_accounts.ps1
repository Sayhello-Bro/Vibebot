...
$failedCount = 0
$uniqueReplies = [System.Collections.Generic.HashSet[string]]::new(
...
function Set-DefaultCandidateEnabled {
    ...
    Invoke-JsonRequest -Method Patch `
        -Uri "$DataApiBase/crowd_slogans/by_text" `
...
function Set-UserCandidateEnabled {
    ...
    Invoke-JsonRequest -Method Patch `
        -Uri "$DataApiBase/crowd_slogans/$Id" `
...
function Initialize-TestCandidates {
    ...
    $script:defaultSnapshot = @(
        (Invoke-RestMethod "$DataApiBase/crowd_slogans?limit=500").items
    )
    $script:userSnapshot = @()
    ...
    foreach ($text in $DesiredCandidates) {
        ...
        $created = Invoke-JsonRequest -Method Post `
            -Uri "$DataApiBase/crowd_slogans" `
            -Body @{
                text = $text
                weight = 1.0
                enabled = $true
                meaning = "直播主指定的衝人氣或下單口令"
                response_mode = "same"
            }
    ...
    $enabledTexts = @(
        $cache.items |
            Where-Object { $_.candidate_type -eq "crowd_slogan" } |
    ...
    Write-Host "目前啟用衝人氣口令（revision=$($cache.candidate_revision)）：" `
...
function Restore-Candidates {
    ...
    foreach ($id in $script:createdIds) {
        try {
            Invoke-RestMethod -Method Delete "$DataApiBase/crowd_slogans/$id" |
                Out-Null
        }
        catch {
...
try {
    ...
    if ($AccountMode -ne "vector_only" -and -not [bool]$replyHealth.qwen_enabled) {
        throw "ENABLE_QWEN=false，qwen_only 測試無法執行。請設為 true 後重啟直播 API。"
    }
    if (
        $AccountMode -ne "vector_only" -and
        -not [string]::IsNullOrWhiteSpace([string]$replyHealth.inference_runtime.error)
    ) {
        $ollamaError = [string]$replyHealth.inference_runtime.error
        throw "Ollama/Qwen 尚未就緒：$ollamaError。請先啟動 Ollama、確認 qwen3:8b 已安裝，再重新啟動 live_stream_llm.py。"
    }
    ...
    foreach ($line in Get-Content -LiteralPath $JsonlPath -Encoding UTF8) {
        ...
        try {
            ...
            if ($AccountMode -eq "qwen_only") {
                ...
                if (-not [bool]$result.qwen_called -and -not [bool]$result.model_input_pending) {
                    throw "qwen_only 應呼叫 Qwen，但 API 回報未呼叫。"
                }
                if (
                    [bool]$result.qwen_called -and
                    ($null -eq $result.generation -or -not [bool]$result.generation.valid)
                ) {
                    $generationReason = [string]$result.generation.validation_reason
                    $generationError = [string]$result.generation.error
                    throw "Qwen 生成失敗：reason=$generationReason；error=$generationError"
                }
                if ([int]$result.crowd_slogan_count -ne $DesiredCandidateCount) {
                    throw "Qwen 應收到 $DesiredCandidateCount 個衝人氣口號，實際為 $($result.crowd_slogan_count)。"
                }
                if (
                    [bool]$result.qwen_called -and
                    [int]$result.generation.crowd_slogan_count_sent -ne $DesiredCandidateCount
                ) {
                    throw "實際送入Qwen的衝人氣口號應為 $DesiredCandidateCount 個，目前為 $($result.generation.crowd_slogan_count_sent)。"
                }
            }
            $processedCount++
            Write-Host (
            ...
            Write-Host (
                "向量執行={0}，向量候選={1}，自訂語句={2}，衝人氣口號={3}，Qwen呼叫={4}，模型累積字數={5}/{6}" -f `
                [bool]$result.vector_search_performed,
                [int]$result.candidate_count,
                [int]$result.reference_candidate_count,
                [int]$result.crowd_slogan_count,
                [bool]$result.qwen_called,
                [int]$result.model_input_char_count,
                [int]$result.model_min_input_chars
            )
            Write-Host (
                "全體衝人氣={0}，基礎口令={1}，各帳號內容={2}" -f `
                [bool]$result.crowd_response,
                $(if ($null -ne $result.crowd_base_token) { [string]$result.crowd_base_token } else { "無" }),
                $(if (@($result.crowd_tokens).Count) { @($result.crowd_tokens) -join "、" } else { "無" })
            )
            ...
            if ([bool]$result.qwen_called) {
                Write-Host (
                    "模型回覆保底={0}次 | 本次保底帳號={1}" -f `
                    [int]$result.reply_frequency.interval,
                    $(if (@($result.reply_frequency.forced_accounts).Count) {
                        @($result.reply_frequency.forced_accounts) -join "、"
                    } else { "無" })
                ) -ForegroundColor DarkGray
            }
            foreach ($accountResult in @($result.account_results)) {
                ...
                if (
                    ...
                    [string]$accountResult.reply_source -notin @("qwen", "qwen_ignore", "qwen_crowd", "qwen_frequency_fallback", "model_input_pending")
                ) {
                    throw "帳號 $($accountResult.account_id) 並非由 Qwen 決定：$($accountResult.reply_source)"
                }
                if ([bool]$accountResult.has_reply) {
                    $replyCount++
                    if (-not $uniqueReplies.Add([string]$accountResult.reply)) {
                        $duplicateReplyCount++
                    }
                }
                $styleName = [string]$accountResult.style
                if (-not $styleCounts.ContainsKey($styleName)) {
                ...
                Write-Host (
                    "帳號={0} | 模式={1} | 風格={2} | 來源={3} | 留言={4} | 回覆片段={5} | 判定原因={6} | 處理時間={7:N2} ms" -f `
                    [string]$accountResult.account_id,
                    [string]$accountResult.mode,
                    [string]$accountResult.style,
            ...
            foreach ($accountResult in @($result.account_results)) {
                ...
                Add-LiveTestReportRow -Report $testReport -Row ([ordered]@{
                    ...
                    crowd_base_token = [string]$result.crowd_base_token
                    crowd_attributes = @($accountResult.model_result.crowd_attributes) -join ","
            ...
            if ($null -ne $result.generation) {
                Write-Host (
                    "Qwen批次結果={0} 筆 | Qwen時間={1:N2} ms | 輸入token={2} | 輸出token={3} | 傳入候選={4} | 重試={5} | temperature={6}" -f `
                    ...
                    [int]$result.generation.retry_count,
                    [double]$result.generation.temperature
                ) -ForegroundColor DarkGray
            }
            Write-Host (
            ...
            if ($_.Exception.Message -like "Qwen 生成失敗：*") {
                throw
            }
        }
    }
    ...
    Write-Host "唯一留言數：$($uniqueReplies.Count)"
    Write-Host "重複留言次數：$duplicateReplyCount"
    Write-Host "風格抽取統計："
    foreach ($styleName in @("reaction", "question", "crowd_response")) {
        $count = if ($styleCounts.ContainsKey($styleName)) { $styleCounts[$styleName] } else { 0 }
        Write-Host "  $styleName = $count"
    }
