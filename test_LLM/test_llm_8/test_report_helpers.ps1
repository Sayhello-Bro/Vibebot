...
function Format-CrowdSignalHint {
    ...
    if ($null -eq $Generation -or $null -eq $Generation.crowd_signal_hints) {
        return "attention=none | allowed=none | viewer_tokens=none | explicit=none"
    }
    ...
    $allowed = @($hints.active_tokens)
    $exact = @($hints.exact_tokens)
    $contextual = @(
        $hints.contextual_attributes |
            ForEach-Object { "{0}:{1}" -f $_.type, $_.value }
    )
    return "attention={0} | allowed={1} | exact={2} | keywords={3} | viewer_tokens={4} | contextual={5} | explicit={6}" -f `
        [string]$hints.attention,
        $(if ($allowed.Count) { $allowed -join "," } else { "none" }),
        $(if ($exact.Count) { $exact -join "," } else { "none" }),
        $(if ($counts.Count) { $counts -join "," } else { "none" }),
        $(if ($tokens.Count) { $tokens -join "," } else { "none" }),
        $(if ($contextual.Count) { $contextual -join "," } else { "none" }),
