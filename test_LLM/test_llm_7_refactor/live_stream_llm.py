...
STYLE_WEIGHTS = {
    "reaction": int(os.environ.get("STYLE_WEIGHT_REACTION", "2")),
    "question": int(os.environ.get("STYLE_WEIGHT_QUESTION", "1")),
}
ACCOUNT_MODES = {"qwen_only", "hybrid", "vector_only"}
DEFAULT_ACCOUNT_MODE = os.environ.get("DEFAULT_ACCOUNT_MODE", "hybrid").strip().lower()
...
def get_stream_reply_mode(stream_id: str) -> str:
    ...
    mode = str(setting.get("reply_mode") or "fixed").strip().lower()
...
def get_stream_enabled_keys(stream_id: str) -> set[str]:
    ...
    cached = candidate_search.list_cached()
    if setting is None:
        return {str(item["key"]) for item in cached}
...
def recent_stream_replies(stream_id: str, limit: int = 5) -> list[str]:
    with STATE_LOCK:
        flattened = [
            reply
...
def choose_fixed_replies(
    raw_text: str, account_ids: list[str], stream_id: str
) -> dict[str, Any]:
    """Compatibility path for the launcher's per-stream checked phrases."""

    started = time.perf_counter()
    enabled_keys = get_stream_enabled_keys(stream_id)
    raw_embedding = get_embedding(raw_text)
    candidates: list[dict[str, Any]] = []
    for item in candidate_search.list_cached(include_embedding=True):
        if str(item["key"]) not in enabled_keys:
            continue
        similarity = cosine_similarity(raw_embedding, item.get("embedding", []))
        if similarity < SIMILARITY_THRESHOLD:
            continue
        candidate = dict(item)
        candidate["similarity"] = similarity
        candidate["score"] = similarity * float(item.get("weight", 1.0))
        candidate.pop("embedding", None)
        candidates.append(candidate)
    candidates.sort(key=lambda item: item["score"], reverse=True)
    candidates = candidates[:MAX_CANDIDATES]
    account_results = assign_replies(account_ids, candidates)
    for item in account_results:
        item.update({"mode": "fixed", "style": "fixed", "reply_source": "fixed"})
    return {
        ...
        "account_modes": {account_id: "fixed" for account_id in account_ids},
        "reply_mode": "fixed",
        "candidate_count": len(candidates),
        "best_similarity": max(
            (item["similarity"] for item in candidates), default=0.0
        ),
        ...
        "generation_cooldown_remaining": 0.0,
...
def _choose_for_accounts_unlocked(
    ...
    context = context or {}
    if account_modes is None:
        if get_stream_reply_mode(stream_id) == "fixed":
            return choose_fixed_replies(raw_text, account_ids, stream_id)
        account_modes = {account_id: "hybrid" for account_id in account_ids}
    ...
    qwen_account_ids = [
        account_id
        for account_id in account_ids
        if account_modes[account_id] in {"qwen_only", "hybrid"}
    ]
    model_input_text = ""
    ...
    vector_account_ids = [
        account_id
        for account_id in account_ids
        if account_modes[account_id] == "vector_only"
        or (account_modes[account_id] == "hybrid" and not model_input_pending)
    ]
    if vector_account_ids:
        search_result = candidate_search.search(
            model_input_text if model_input_ready else raw_text,
            limit=MAX_CANDIDATES,
            stream_id=stream_id,
            product_type=product_type,
        )
        fixed_candidates = search_result["candidates"]
        vector_search_performed = bool(search_result["learning_ready"])
    else:
    ...
    # Hybrid and vector-only accounts consume vector candidates first. Qwen
    # still sees every model-capable account so it can make one global crowd
    # decision; when that decision is false, vector-hit Hybrid results win.
    vector_by_account = {
        item["account_id"]: item
        for item in assign_replies(vector_account_ids, fixed_candidates)
    }
    reference_candidates = (
        candidate_search.list_cached() if qwen_account_ids else []
    )
    ...
    crowd_token = (
        str(generation.get("crowd_token") or "") if crowd_response else None
    )
    account_results: list[dict[str, Any]] = []
    for account_id in account_ids:
        mode = account_modes[account_id]
        ...
        if crowd_response and crowd_token:
            selected = {
        ...
        if (
            mode in {"hybrid", "vector_only"}
            and vector_item
            and vector_item["has_reply"]
        ):
            account_results.append(
                {
                    **vector_item,
        ...
        # Qwen-only and vector-miss hybrid accounts never fall back after the
        # model stage because vector search already ran first.
    ...
    if qwen_called and generation and generation.get("valid") and product_type:
        try:
            generated_examples = [
                {
                    **item,
...
def print_processing_result(result: dict[str, Any]) -> None:
    """Show one readable diagnostic block in the single LLM monitor console."""

    raw_text = single_line_log_value(result.get("raw_text"))
    current_chars = int(result.get("model_input_char_count") or 0)
    minimum_chars = int(result.get("model_min_input_chars") or 0)
    print("-" * 72, flush=True)
    print(
        f"{result.get('timestamp', get_current_time())} "
        f"[{result.get('stream_id', 'unknown')}] 收到句子：{raw_text}",
        flush=True,
    )
    if minimum_chars:
        print(
            f"模型累積：{current_chars}/{minimum_chars} 字｜"
            f"Qwen 呼叫：{'是' if result.get('qwen_called') else '否'}",
            flush=True,
        )
    for item in result.get("account_results", []):
        reply = single_line_log_value(item.get("reply")) or "ignore"
        source = single_line_log_value(item.get("reply_source")) or "unknown"
        print(
            f"帳號 {item.get('account_id', 'unknown')}｜"
            f"結果來源：{source}｜回覆：{reply}",
            flush=True,
        )
...
def stream_replies():
    """Keep the desktop launcher's fixed/automatic controls API-compatible."""
    ...
    try:
        ...
        if setting is None:
            enabled_keys = {
                str(item["key"]) for item in candidate_search.list_cached()
            }
            reply_mode = "fixed"
        else:
            enabled_keys = {str(key) for key in setting.get("enabled_keys", [])}
        ...
        if request.method == "PATCH":
            ...
            if requested_mode:
                if requested_mode not in {"fixed", "auto"}:
                    return jsonify(
                        {
                            "status": "error",
                            "error": "reply_mode must be fixed or auto",
                        }
                    ), 400
            ...
            if not reply_key and not requested_mode:
                return jsonify(
                    {
                        "status": "error",
                        "error": "reply_key or reply_mode is required",
                    }
                ), 400
            stream_settings_collection.update_one(
                ...
                {
                    "$set": {
                        "enabled_keys": sorted(enabled_keys),
        ...
        return jsonify(
            {
                ...
                "enabled_keys": sorted(enabled_keys),
                "reply_mode": reply_mode,
                "configured": setting is not None or request.method == "PATCH",
            }
        )
    except Exception as exc:
        return jsonify({"status": "error", "error": str(exc)}), 500
...
def stream_product():
    try:
        ...
        if request.method in {"POST", "PUT"}:
            ...
            if not product_type:
                return jsonify(
                    {"status": "error", "error": "product_type is required"}
                ), 400
