    ...
    foreach ($line in Get-Content -LiteralPath $JsonlPath -Encoding UTF8) {
        ...
        try {
            ...
            if ($AccountMode -eq "qwen_only") {
                ...
                if ([int]$result.reference_candidate_count -ne $DesiredCandidateCount) {
            ...
            Write-Host (
                "向量執行={0}，向量候選={1}，模型參考候選={2}，Qwen呼叫={3}，模型累積字數={4}/{5}" -f `
            ...
            foreach ($accountResult in @($result.account_results)) {
                ...
                if (
                    ...
                    [string]$accountResult.reply_source -notin @("qwen", "qwen_ignore", "qwen_crowd", "model_input_pending")
                ) {
